...
import com.senac.SistemaBancario.model.Cliente;
import com.senac.SistemaBancario.model.Conta;
import com.senac.SistemaBancario.model.Especial;
import com.senac.SistemaBancario.model.Investimento;
import com.senac.SistemaBancario.model.SaldoInsuficienteException;
import com.senac.SistemaBancario.view.ConsoleView;
...
public class SistemaBancario {
	...
	public void run() {
		String[] opcoes = {"Cria Conta Comum",
				           "Cria Conta Especial",
				           "Cria Conta Investimento",
				           "Exclui Conta",
				           "Gerencia Conta",
		                   "Fim do Programa"};
		int option = 0;
		while (option != opcoes.length) {
			...
			switch (option) {
				...
				case 2: criaContaEspecial(); break;
				case 3: criaContaInvestimento(); break;
				case 4: excluiConta(); break;
				case 5: gerenciaConta(); break;
				default: view.showMessage("Opcao Invalida.");
			}
		}
	}
	...
	private Conta procuraConta() {
		...
		for (int i = 0; i < contas.length; i++) {
			if (contas[i] != null && contas[i].getCliente().getCPF().equals(cpf)) {
				return contas[i];
			}
		}
		return null;
	...
	private void opcoesConta(Conta conta) {
		if (conta instanceof Investimento) {
			String[] menuInvestimento = {"Saldo", "Sacar", "Depositar", "Dividendos"};
			int option = view.getMenuOption("Operacoes de Conta Investimento", menuInvestimento);
			switch (option) {
				case 1: saldo(conta); break;
				case 2: sacar(conta); break;
				case 3: depositar(conta); break;
				case 4: dividendos((Investimento)conta); break;
				default: view.showMessage("Opcao invalida");
			}
		} else {
